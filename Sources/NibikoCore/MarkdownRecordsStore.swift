...
public struct MarkdownRecordsStore: Equatable, Sendable {
    private let recordsFileName: String
    private let recordBlockStart = "<!-- nibikoRecord -->"
    private let recordBlockEnd = "<!-- /nibikoRecord -->"
    private let legacyRecordBlockStart = "<!-- menuBarNotesRecord -->"
    private let legacyRecordBlockEnd = "<!-- /menuBarNotesRecord -->"
    private let legacySharedFileName = "MenuBarNotes.md"

    public init(recordsFileName: String = AppSettings.defaultMarkdownStorageFileName) {
    ...
    public func loadRecords(in directory: URL) throws -> [Record] {
        ...
        } else if recordsFileName == AppSettings.recordsFileName {
            let legacySharedFileURL = directory.appendingPathComponent(legacySharedFileName)
            let legacySharedRecords = FileManager.default.fileExists(atPath: legacySharedFileURL.path)
                ? loadLegacySharedRecords(at: legacySharedFileURL)
                : []
            if !legacySharedRecords.isEmpty {
                try? writeRecords(legacySharedRecords, in: directory)
                records = legacySharedRecords
            } else {
        ...
        return fileURLs
            .filter {
                $0.pathExtension.lowercased() == "md"
                    && $0.lastPathComponent != recordsFileName
                    && $0.lastPathComponent != legacySharedFileName
            }
            .compactMap { try? parseRecord(at: $0) }
    }

    private func loadLegacySharedRecords(at fileURL: URL) -> [Record] {
        let records = (try? parseRecordsFile(at: fileURL)) ?? []
        if !records.isEmpty {
            return records
        }

        return (try? parseRecord(at: fileURL)).map { [$0] } ?? []
    }
    ...
    private func parseRecordsFile(at fileURL: URL) throws -> [Record] {
        ...
        return parseRecordBlocks(in: text, startMarker: recordBlockStart, endMarker: recordBlockEnd)
            .compactMap { try? parseRecord(from: $0) }
            + parseRecordBlocks(in: text, startMarker: legacyRecordBlockStart, endMarker: legacyRecordBlockEnd)
                .compactMap { try? parseRecord(from: $0) }
    }

    private func parseRecordBlocks(in text: String, startMarker: String, endMarker: String) -> [String] {
        let lines = text.components(separatedBy: .newlines)
        var blocks: [String] = []
        var index = lines.startIndex

        while index < lines.endIndex {
            guard lines[index] == startMarker else {
                index = lines.index(after: index)
                continue
            }

            let blockStart = lines.index(after: index)
            guard let blockEnd = lines[blockStart...].firstIndex(of: endMarker) else {
                break
            }
    ...
    private func parseRecord(from text: String) throws -> Record {
        ...
        guard
            frontMatter["nibikoRecord"] == "v1" || frontMatter["menuBarNotesRecord"] == "v1",
