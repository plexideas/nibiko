...
public enum RecordDisplaySupport {
    ...
    public static func isOverdue(_ record: Record, now: Date = Date()) -> Bool {
        guard
            record.kind == .reminder,
            record.status == .active,
            let displayTime = displayTime(for: record)
        else {
            return false
        }
    ...
    public static func hasOverdueReminder(_ records: [Record], now: Date = Date()) -> Bool {
        records.contains { record in
            isOverdue(record, now: now)
        }
    }
}
