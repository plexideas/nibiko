...
struct MarkdownRecordsStoreTests {
    ...
    func writesAndReadsRecords() throws {
        ...
        #expect(markdownFiles.map(\.lastPathComponent) == ["storage.md"])
    }
    ...
    @Test("storage Markdown file migrates records from the legacy shared file")
    func storageFileMigratesLegacySharedFile() throws {
        let directory = try temporaryDirectory()
        let store = MarkdownRecordsStore()

        try legacyRecordMarkdown(id: "legacy-shared", kind: .note, title: "Existing note", createdAt: Date())
            .write(
                to: directory.appendingPathComponent("Nibiko.md"),
                atomically: true,
                encoding: .utf8
            )
        let records = try store.loadRecords(in: directory)
        let migratedMarkdown = try String(
            contentsOf: directory.appendingPathComponent("storage.md"),
            encoding: .utf8
        )

        #expect(records.map(\.id) == ["legacy-shared"])
        #expect(migratedMarkdown.contains("id: legacy-shared"))
    }
